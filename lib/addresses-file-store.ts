...
export function loadFromFile(userId: string): AddressesFileData | null {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[addresses-file-store] loadFromFile userId:', userId, 'path:', filePath, 'exists:', exists);
    }
...
export function saveToFile(userId: string, data: AddressesFileData): void {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[addresses-file-store] saveToFile ok userId:', userId, 'path:', filePath, 'addresses:', data.addresses?.length ?? 0);
    }
  } catch (err) {
    if (process.env.NODE_ENV === 'development') {
      console.warn('[addresses-file-store] saveToFile failed:', (err as Error)?.message ?? err, 'path:', getFilePath(userId));
    }
  }
}
