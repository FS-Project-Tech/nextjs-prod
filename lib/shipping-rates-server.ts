...
// /**
//  * AU 4207–4228 (Gold Coast local): checkout shows only generic free shipping, Express flat rate, and pickup.
//  * No minimum-purchase rules apply for these postcodes (rates omit minimum_amount / min_amount requires).
//  */
// function isGoldCoastLocalPostcodeRange(postcode: string): boolean {
//   const digits = String(postcode || "").replace(/\D/g, "");
//   if (digits.length < 4) return false;
//   const n = parseInt(digits.slice(0, 4), 10);
//   return !Number.isNaN(n) && n >= 4207 && n <= 4228;
// }

// /**
//  * Allowlist for Gold Coast local postcodes: drop "$50 ex GST" free-shipping instance and Standard flat rate.
//  */
// function allowedGoldCoastLocalShippingMethod(methodId: string | undefined, label: string): boolean {
//   const id = (methodId || "").toLowerCase();
//   const lb = (label || "").toLowerCase();
//   if (id === "local_pickup") return true;
//   if (id === "free_shipping") {
//     if (lb.includes("offered on orders valued at") || lb.includes("ex gst")) return false;
//     return true;
//   }
//   if (id === "flat_rate") {
//     if (
//       lb.includes("standard delivery") ||
//       lb.includes("1-5 business") ||
//       (lb.includes("1-5") && lb.includes("business"))
//     )
//       return false;
//     if (lb.includes("express")) return true;
//     return false;
//   }
//   return false;
// }
...
//   const goldCoastLocalAu = country === "AU" && isGoldCoastLocalPostcodeRange(postcode);
...
//       let shouldInclude = true;
//       if (!skipMinAmountForFree && !goldCoastLocalAu) {
//         if (minimum_amount !== undefined && cartSubtotal < minimum_amount) shouldInclude = false;
//         if (requires === "min_amount" && (!minimum_amount || cartSubtotal < minimum_amount))
//           shouldInclude = false;
//         if (
...
//           shouldInclude = false;
//         if (maximum_amount !== undefined && cartSubtotal > maximum_amount) shouldInclude = false;
//       } else if (!skipMinAmountForFree && goldCoastLocalAu) {
//         if (maximum_amount !== undefined && cartSubtotal > maximum_amount) shouldInclude = false;
//       }
...
//         shouldInclude &&
//         country === "AU" &&
//         isGoldCoastLocalPostcodeRange(postcode) &&
//         !allowedGoldCoastLocalShippingMethod(row.method_id, label)
//       ) {
//         shouldInclude = false;
//       }
...
//       if (shouldInclude) {
//         const rateRequires =
//           goldCoastLocalAu && requires === "min_amount" ? undefined : requires;
//         methods.push({
//           id: `${row.method_id || row.id}:${row.instance_id}`,
//           label: label || row.title || row.method_title || String(row.id),
//           cost: isNaN(cost) ? 0 : cost,
//           zoneId: z.id,
//           zone: z.name,
//           minimum_amount: goldCoastLocalAu ? undefined : minimum_amount,
//           maximum_amount,
//           requires: rateRequires,
//           description,
//         });
//       }
//     }
