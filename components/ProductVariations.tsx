...
function variationIsOnSale(variation: WooCommerceVariation): boolean {
  const regular = Number(variation.regular_price || 0);
  const sale = Number(variation.sale_price || 0);
  return Boolean(variation.on_sale) || (regular > 0 && sale > 0 && sale < regular);
}
...
export default function ProductVariations({
  ...
  const optionHasSaleVariation = (attributeName: string, option: string): boolean => {
    ...
    return variations.some((variation) => {
      if (variation.stock_status === "outofstock" || !variationIsOnSale(variation)) return false;

      return Object.entries(candidateSelected).every(([selectedName, selectedValue]) => {
        if (!selectedValue) return true;
        const variationAttr = variation.attributes.find((attr) =>
          attributeNameMatches(attr.name, selectedName)
        );
        return (
          variationAttr &&
          variationOptionMatchesSelected(variationAttr.option, selectedValue)
        );
      });
    });
  };
  ...
  const renderSwatch = (
    ...
    const hasSale = isEnabled && optionHasSaleVariation(attributeName, option);
    ...
    return (
      <button
        ...
        disabled={!isEnabled}
        className={`relative rounded-md border px-4 py-2 text-sm font-medium transition-all cursor-pointer ${
          isSelected
      ...
      >
        {hasSale ? (
          <span className="absolute -right-2 -top-2 rounded-full bg-red-600 px-1.5 py-0.5 text-[10px] font-bold leading-none text-white shadow-sm">
            Sale
          </span>
        ) : null}
        {option}
    ...
    return (
      ...
      >
        ...
        {allOptions.map((option) => {
          ...
          const hasSale = canPick && optionHasSaleVariation(attributeName, option);
          return (
            <option key={option} value={option} disabled={!canPick}>
              {!canPick ? `${option} (unavailable)` : hasSale ? `${option} (Sale)` : option}
            </option>
          );
        })}
      </select>
