...
const WP_SLUG = "medical-supplies-toowoomba";
...
async function loadPage() {
  try {
    ...
    console.error("[medical-supplies-toowoomba] fetchPageBySlug threw", {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    console.error("[medical-supplies-toowoomba] generateMetadata: fetch failed", {
      message: err instanceof Error ? err.message : String(err),
    });
  }
  ...
  const rawTitle = page?.title?.rendered
    ? String(page.title.rendered)
        ...
        .trim()
    : "";
  const title = rawTitle ? decodeHTMLEntities(rawTitle) : "Medical Supplies Toowoomba";
  ...
  const pagePath = "/medical-supplies-toowoomba";
...
export default async function MedicalSuppliesToowoombaPage() {
  const page = await loadPage();
  if (!page) {
    console.error("CMS page not found:", WP_SLUG);
    return <CmsPageFallback slug={WP_SLUG} breadcrumbLabel="Medical Supplies Toowoomba" />;
  }

  const title = decodeHTMLEntities(
    page.title?.rendered?.replace(/<[^>]+>/g, "").trim() || "Medical Supplies Toowoomba"
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        <section className="container mx-auto px-4 pb-12 sm:px-6 md:px-8 md:pb-16">
          <div
            className="medical-supplies-toowoomba-page-content mx-auto max-w-8xl text-gray-900"
