...
const WP_SLUG = "medical-supplies-adelaide";
...
async function loadPage() {
  try {
    ...
    console.error("[medical-supplies-adelaide] fetchPageBySlug threw", {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    console.error("[medical-supplies-adelaide] generateMetadata: fetch failed", {
      message: err instanceof Error ? err.message : String(err),
    });
  }
  ...
  const rawTitle = page?.title?.rendered
    ? String(page.title.rendered)
        ...
        .trim()
    : "";
  const title = rawTitle ? decodeHTMLEntities(rawTitle) : "Medical Supplies Adelaide";
  ...
  const pagePath = "/medical-supplies-adelaide";
...
export default async function MedicalSuppliesAdelaidePage() {
  const page = await loadPage();
  if (!page) {
    console.error("CMS page not found:", WP_SLUG);
    return <CmsPageFallback slug={WP_SLUG} breadcrumbLabel="Medical Supplies Adelaide" />;
  }

  const title = decodeHTMLEntities(
    page.title?.rendered?.replace(/<[^>]+>/g, "").trim() || "Medical Supplies Adelaide"
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        <section className="container mx-auto px-4 pb-12 sm:px-6 md:px-8 md:pb-16">
          <div
            className="medical-supplies-adelaide-page-content mx-auto max-w-8xl text-gray-900"
