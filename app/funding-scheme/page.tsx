...
function parseFundingContent(html: string): { intro: string; cards: FundingCard[] } {
  ...
  for (const s of cardSections) {
    ...
    else if (titleLower.includes("aged care") || titleLower.includes("home care")) slug = "my-aged-care";
    else if (titleLower.includes("caps") || titleLower.includes("continence")) slug = "caps";
 
    const linkMatch = s.content.match(/<a\s[^>]*href=["'][^"']*\/funding-scheme\/([^/"']+)[^"']*["'][^>]*>/i);
    const extractedSlug = linkMatch?.[1];
    const finalSlug = extractedSlug && ["caps", "my-aged-care", "ndis"].includes(extractedSlug) ? extractedSlug : slug;
...
export default async function FundingSchemePage() {
  ...
  const bannerDesc = intro
    ? decodeHTMLEntities(intro.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim())
    : "";
      ...
      <div className="lg:w-2/5 shrink-0">
        {featuredImg ? (
          <div className="relative aspect-[4/3] overflow-hidden">
            <Image src={featuredImg} alt={title} width={1000} height={1000}className="mt-23 rounded-lg" />
          </div>
        ) : (
