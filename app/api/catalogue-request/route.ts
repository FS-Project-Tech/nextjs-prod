...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!reason_for_ordering) {
      return secureResponse(
        { error: "Reason for ordering is required." },
        { status: 400 }
      );
    }
    ...
    const adminTo =
      process.env.CATALOGUE_REQUEST_ADMIN_EMAIL?.trim() || ADMIN_EMAIL_DEFAULT;
        ...
        if (customUrl) {
          ...
          if (
            ...
            return secureResponse(
              {
                error:
                  "We could not save your catalogue request. Please try again or contact us.",
