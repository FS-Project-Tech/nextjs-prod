...
const WP_SLUG = "medical-supplies-brisbane-north";
...
async function loadPage() {
  try {
    ...
    console.error("[medical-supplies-brisbane-north] fetchPageBySlug threw", {
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  try {
    ...
    console.error("[medical-supplies-brisbane-north] generateMetadata: fetch failed", {
      message: err instanceof Error ? err.message : String(err),
    });
  }
  ...
  const rawTitle = page?.title?.rendered
    ? String(page.title.rendered)
        ...
        .trim()
    : "";
  const title = rawTitle ? decodeHTMLEntities(rawTitle) : "Medical Supplies Brisbane North";
  ...
  const pagePath = "/medical-supplies-brisbane-north";
...
export default async function MedicalSuppliesBrisbaneNorthPage() {
  const page = await loadPage();
  if (!page) {
    console.error("CMS page not found:", WP_SLUG);
    return <CmsPageFallback slug={WP_SLUG} breadcrumbLabel="Medical Supplies Brisbane North" />;
  }

  const title = decodeHTMLEntities(
    page.title?.rendered?.replace(/<[^>]+>/g, "").trim() || "Medical Supplies Brisbane North"
  );
    ...
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        <section className="container mx-auto px-4 pb-12 sm:px-6 md:px-8 md:pb-16">
          <div
            className="medical-supplies-brisbane-north-page-content mx-auto max-w-8xl text-gray-900"
